...
export type PartnershipApplicationLocale = "en" | "zh" | "ar" | "id";
...
export function getPartnershipApplicationDefinition(
  ...
  // Arabic and Indonesian use the same validated field schema while the
  // surrounding page and form chrome are localized in the component. This
  // keeps submitted payload keys stable as we expand field-level copy.
  const localizedCopy = (copy as Partial<Record<PartnershipApplicationLocale, typeof copy.en>>)[locale] ?? copy.en;
  const definition = localizedCopy[type];
  if (locale !== "ar" && locale !== "id") {
    return definition;
  }

  const labels = locale === "ar"
    ? {
        organizationName: ["اسم المؤسسة", "اسم المستشفى أو الشركة"], website: ["الموقع الإلكتروني", "https://example.com"], contactName: ["جهة الاتصال الرئيسية", "الاسم الكامل"],
        jobTitle: ["المنصب", "المسمى الوظيفي"], email: ["البريد الإلكتروني للعمل", "partner@example.com"], phone: ["الهاتف", "+86 ..."], whatsapp: ["واتساب", "+1 ..."], wechat: ["WeChat", "معرّف WeChat"], country: ["الدولة", "الدولة"], city: ["المدينة الرئيسية", "شنغهاي"], notes: ["ملاحظات إضافية", "أضف أي معلومات أخرى"],
        services: ["الأقسام أو الإجراءات الرئيسية", "اذكر التخصصات والإجراءات الرئيسية"], readiness: ["جاهزية خدمة المرضى الدوليين", "صف المنسقين ودعم اللغات ومسار القبول"], credentials: ["التراخيص أو الاعتمادات", "اذكر التراخيص والشهادات ذات الصلة"], referralVolume: ["حجم الإحالات المتوقع", "عدد المرضى المتوقع سنوياً"], languages: ["اللغات المدعومة", "الإنجليزية والعربية والروسية وغيرها"], serviceCategory: ["فئة الخدمة", "اختر فئة الخدمة"], coverage: ["مدن التغطية", "المدن التي تقدمون فيها الخدمة"], experience: ["خبرة خدمة المسافرين للعلاج", "صف خبرتكم مع المرضى الدوليين"],
      }
    : {
        organizationName: ["Nama organisasi", "Nama rumah sakit atau perusahaan"], website: ["Situs web", "https://example.com"], contactName: ["Kontak utama", "Nama lengkap"],
        jobTitle: ["Jabatan", "Posisi pekerjaan"], email: ["Email kerja", "partner@example.com"], phone: ["Telepon", "+86 ..."], whatsapp: ["WhatsApp", "+1 ..."], wechat: ["WeChat", "ID WeChat"], country: ["Negara", "Negara"], city: ["Kota utama", "Shanghai"], notes: ["Catatan tambahan", "Tambahkan informasi lain"],
        services: ["Departemen atau prosedur utama", "Sebutkan spesialisasi dan prosedur utama"], readiness: ["Kesiapan pasien internasional", "Jelaskan koordinator, dukungan bahasa, dan alur penerimaan"], credentials: ["Lisensi atau kredensial", "Sebutkan lisensi dan sertifikasi terkait"], referralVolume: ["Perkiraan volume rujukan", "Jumlah pasien yang diperkirakan per tahun"], languages: ["Bahasa yang didukung", "Bahasa Inggris, Arab, Rusia, dan lainnya"], serviceCategory: ["Kategori layanan", "Pilih kategori layanan"], coverage: ["Kota cakupan", "Kota tempat Anda menyediakan layanan"], experience: ["Pengalaman melayani wisatawan medis", "Jelaskan pengalaman dengan pasien internasional"],
      };

  return {
    ...definition,
    fields: definition.fields.map((field) => {
      const copyForField = labels[field.id as keyof typeof labels];
      return copyForField ? { ...field, label: copyForField[0], placeholder: copyForField[1] } : field;
    }),
  };
}
