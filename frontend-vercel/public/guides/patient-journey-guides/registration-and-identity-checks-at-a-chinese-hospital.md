...
Create a one-page table using the current passport as the anchor:

| Field | What to record |
|---|---|
| passport display name | exact order, spaces, hyphens, apostrophes and diacritics |
| machine-readable name | spelling shown in the MRZ at the bottom of the passport page |
| primary/family name | as designated by the passport issuer |
| given names | all components, including middle names |
| date of birth | write as `YYYY-MM-DD` and also show the original document format |
| document | passport number, issuing country and expiry date |
| previous identity | former passport number, former legal name, maiden name or alias |
...
Before a new account or card is issued, ask staff to search using:

- current and previous passport numbers;
- full passport name and common system variants;
- date of birth and sex as recorded;
...
1. Which number is the hospital's permanent medical record identifier?
2. Which app or phone account is linked to it?
...
Name plus date of birth is a common pair, but a hospital may use the medical record number, passport number or another person-specific identifier. WHO patient-identification guidance recommends at least two identifiers and matching the correct person to medicines, specimens, procedures and results [5].

At each boundary, the patient should state the answers rather than nodding:

- before blood draw or specimen collection;
- before imaging, endoscopy or a procedure;
- before medicine, infusion or transfusion;
- when a wristband is applied or replaced;
...
For a specimen, verify the label while the patient is present. The label and order should refer to the same patient and the same requested test. Note collection date/time and specimen type when relevant.
...
- dual nationality or two passports;
- names with several family-name components;
- patronymics or matronymics;
- single-name passports;
- adopted names or gender-marker changes;
...
If any identifier is wrong:

1. **pause** the blood draw, medicine, procedure, upload or discharge release if it is safe to do so;
2. **state the mismatch precisely**—for example, correct date of birth but wrong hospital number;
3. **preserve evidence** with a screenshot, printed label or order number without exposing it to unauthorised people;
4. **notify the clinical team and registration/medical-record office**, not only the app help desk;
...
### Audit the chain before leaving China

At the end of an outpatient episode or admission, compare the identity shown on:

- clinical summary and diagnosis list;
- laboratory and pathology reports;
- imaging reports and DICOM headers;
...
1. [National Health Commission — Key Medical Quality and Safety Systems](https://www.nhc.gov.cn/yzygj/c100068/201804/42ab2552298047679cd6ec733f233862.shtml)
2. [National Health Commission — WS/T 840—2025 Patient Identification Management Standard](https://www.nhc.gov.cn/fzs/c100048/202508/ca6d87e1ab7442b99db623c702110c9c/files/WST%20840-2025%E6%82%A3%E8%80%85%E8%BA%AB%E4%BB%BD%E8%AF%86%E5%88%AB%E7%AE%A1%E7%90%86%E6%A0%87%E5%87%86.pdf)
3. [ICAO — Doc 9303, Machine Readable Travel Documents, Part 4](https://www.icao.int/publications/doc-series/doc-9303)
4. [National Health Commission — Functional Specification for Electronic Medical Record Systems](https://www.nhc.gov.cn/wjw/gfxwj/201101/a769b5f4b9ca4415a72fa9888bce0bc1.shtml)
